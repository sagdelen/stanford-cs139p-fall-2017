...
class ViewController: UIViewController {
    ...
    private var faceUpCardViewsMatch: Bool {
        return faceUpCardViews.count == 2 &&
            faceUpCardViews.first!.rank == faceUpCardViews.last!.rank &&
            faceUpCardViews.first!.suit == faceUpCardViews.last!.suit
    }
...
extension CGFloat {
    var arc4random: CGFloat {
        if self > 0 {
            return CGFloat.random(in: 0...1) * self
        } else if self < 0 {
            return -CGFloat.random(in: 0...1) * self
        } else {
            return 0.0
